...
class gatt_if_t {
 public:
  ...
  virtual esp_err_t response(uint16_t conn_id, uint32_t trans_id, esp_gatt_status_t status, esp_gatt_rsp_t* rsp) = 0;

  virtual esp_err_t send_indicate(uint16_t conn_id, uint16_t attr_handle, uint16_t value_len, uint8_t* value,
                                  bool need_confirm) = 0;
};
...
class esp_gatt : public gatt_if_t {
 public:
  ...
  esp_err_t send_indicate(uint16_t conn_id, uint16_t attr_handle, uint16_t value_len, uint8_t* value,
                          bool need_confirm) override;
 private:
